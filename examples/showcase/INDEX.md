...
102. `102_openclaw_wallet_draining_phishing`: OpenClaw wallet-draining phishing campaign using fake CLAW token airdrop offers on GitHub, with C2 at token-claw.xyz and watery-compost.today, as reported by OX Security (`MAL-042`)
103. `103_roguepilot_html_comment_injection`: RoguePilot-style hidden HTML comment prompt injection that embeds invisible instructions in GitHub Issues to hijack AI assistants and exfiltrate GITHUB_TOKEN credentials, as documented by Orca Security (`PINJ-005`)
