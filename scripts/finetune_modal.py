...
gpu_image = (
    ...
    .pip_install(
        ...
        "pyyaml>=6.0",
        "tqdm>=4.66.0",
        "optimum[onnxruntime]>=1.18.0",  # ONNX export
...
def run_finetune(
    ...
    # -----------------------------------------------------------------------
    # ONNX export: merge LoRA → base, export to ONNX FP16
    # FP16 is used instead of INT8 because INT8 (QOperator/avx512_vnni) severely
    # degrades DeBERTa-v3 quality: the relative position attention mechanism is
    # sensitive to quantization error, causing F1 to collapse from 0.84 to 0.32.
    # FP16 gives ~2× size reduction vs FP32 with zero precision loss and better
    # CPU latency than INT8 on AVX-512 for small batch sizes.
    ...
    try:
        ...
        print("Exporting merged model to ONNX (fp32)...")
        ...
        # Convert fp32 model.onnx to fp16 using onnxconverter-common
        quantization_label = "fp32"
        try:
            import onnx
            from onnxconverter_common import float16
            fp32_path = str(_pathlib.Path(onnx_dir) / "model.onnx")
            fp16_path = str(_pathlib.Path(onnx_dir) / "model_fp16.onnx")
            fp32_model_obj = onnx.load(fp32_path)
            fp16_model_obj = float16.convert_float_to_float16(
                fp32_model_obj,
                keep_io_types=True,   # keep input/output as fp32 for compatibility
                disable_shape_infer=False,
            )
            onnx.save(fp16_model_obj, fp16_path)
            fp16_sz = _pathlib.Path(fp16_path).stat().st_size
            fp32_sz = _pathlib.Path(fp32_path).stat().st_size
            print(f"FP16 model: {fp16_sz/1e6:.1f} MB  (fp32 was {fp32_sz/1e6:.1f} MB, {fp32_sz/fp16_sz:.2f}x reduction)")
            quantization_label = "fp16"
        except Exception as fp16_exc:
            print(f"FP16 conversion failed ({fp16_exc}), keeping fp32 model.onnx")
        ...
        onnx_result = {"exported": True, "quantization": quantization_label, "dir": onnx_dir}
        print(f"ONNX {quantization_label.upper()} model ready at {onnx_dir}")
...
def main(
    ...
    # Respect SKILLSCAN_CORPUS_DIR env var (set by CI); fall back to:
    # 1. repo_root/corpus (symlink or real dir)
    # 2. repo_root/training_corpus (actual dir name in skillscan-corpus repo)
    _env_dir = os.environ.get("SKILLSCAN_CORPUS_DIR", "")
    if _env_dir and Path(_env_dir).exists():
        corpus_dir = Path(_env_dir)
    elif (repo_root / "corpus").exists():
        corpus_dir = repo_root / "corpus"
    elif (repo_root / "training_corpus").exists():
        corpus_dir = repo_root / "training_corpus"
    else:
        print("ERROR: corpus/ directory not found. Run: skillscan corpus sync")
        sys.exit(1)
    # manifest.json lives at the repo root, not inside corpus_dir
