...
def test_showcase_137_mal057_jarkastealer_ai_trojan() -> None:
    ...
    report = scan(
        Path("examples/showcase/137_jarkastealer_ai_trojan"), STRICT, "builtin:strict"
    )
    finding_ids = {f.id for f in report.findings}
...
def test_showcase_138_exec042_ide_config_injection() -> None:
    ...
    report = scan(
        Path("examples/showcase/138_ide_config_injection"), STRICT, "builtin:strict"
    )
    finding_ids = {f.id for f in report.findings}
...
def test_showcase_139_pinj018_hidden_prompt_injection() -> None:
    ...
    report = scan(
        Path("examples/showcase/139_hidden_prompt_injection"), STRICT, "builtin:strict"
    )
    finding_ids = {f.id for f in report.findings}
