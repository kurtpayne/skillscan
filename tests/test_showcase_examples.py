...
def test_showcase_detection_rules() -> None:
    ...
    _inv54 = iter_text_files(
        Path("examples/showcase/54_claude_hooks_rce"),
        STRICT.limits["max_files"],
        STRICT.limits["max_bytes"],
        STRICT.limits.get("max_binary_artifacts", 500),
        STRICT.limits.get("max_binary_bytes", 100_000_000),
    )
    print(f"DEBUG files_54: {_inv54.text_files}", flush=True)
    ...
    assert any(f.id == "MAL-015" for f in findings_54), (
        f"MAL-015 not found; files={_inv54.text_files}; got: {[f.id for f in findings_54]}"
    )
