...
- Added `SUP-049` (critical): **Mini Shai-Hulud SAP CAP npm supply chain compromise (@cap-js/sqlite@2.2.2, @cap-js/postgres@2.2.2, @cap-js/db-service@2.10.1, mbt@1.2.48)** — Four SAP CAP ecosystem npm packages were compromised on April 29, 2026 in a Mini Shai-Hulud / TeamPCP supply chain attack. Entry was via a stolen CircleCI npm token obtained from a malicious draft PR (`feat: ci speedup`) against SAP/cloud-mta-build-tool that ran in a PR build exposing project secrets. The compromised packages add a preinstall hook (`node setup.mjs`) that downloads Bun 1.3.13 from GitHub and executes an 11.7 MB obfuscated payload (`execution.js`, labeled `ctf-scramble-v2`). The payload harvests GitHub tokens (`gh auth token`), npm tokens (`.npmrc`), AWS/Azure/GCP/Kubernetes secrets, GitHub Actions runner secrets (via `/proc` memory), Claude config, MCP config, and VPN configs. Stolen data is AES-256-GCM encrypted with RSA-4096 key wrapping and exfiltrated via newly created GitHub repositories with description "A Mini Shai-Hulud has Appeared". The malware propagates by pushing `.vscode/tasks.json`, `.vscode/setup.mjs`, `.claude/execution.js`, `.claude/setup.mjs`, and `.claude/settings.json` into repositories it can access, using commit author `claude <claude@users.noreply.github.com>` and message `chore: update dependencies`. The malware exits cleanly on Russian-locale systems. Rotate ALL secrets if any affected package was installed. SAP Security Note 3747787.
- Added `PSV-059` (critical): **LiteLLM pre-auth SQL injection via Bearer token (CVE-2026-42208, GHSA-r75f-5x8p-qvmc, CVSS 9.3, actively exploited)** — CVE-2026-42208 affects BerriAI LiteLLM versions >= 1.81.16 and < 1.83.7. The Bearer token value in the Authorization header is concatenated directly into a SELECT query against the `LiteLLM_VerificationToken` table without parameter binding, allowing unauthenticated attackers to inject arbitrary SQL via a crafted Authorization header. Exploitation was observed in the wild within 36 hours of disclosure (first attempt April 26, 2026 at 16:17 UTC) from IPs `65.111.27.132` and `65.111.25.67` (AS200373, 3xK Tech GmbH, DE). Attackers targeted tables `litellm_verificationtoken`, `litellm_credentials.credential_values`, and `litellm_config` to extract OpenAI organization keys, Anthropic console keys, AWS Bedrock IAM credentials, and other LLM provider API keys. The fix in v1.83.7-stable replaces string interpolation with parameterized queries. Upgrade immediately to LiteLLM >= 1.83.7. If patching is not immediately possible, set `disable_error_logs: true` under `general_settings`. Treat any internet-facing instance running 1.81.16–1.83.6 as compromised and rotate all stored LLM provider API keys.
- IOC update: added `65.111.27.132` and `65.111.25.67` to IP IOC DB; added GitHub commit search URL for `OhNoWhatsGoingOnWithGitHub` to URL IOC DB.
- Vuln DB update: added `litellm-proxy` (CVE-2026-42208, GHSA-r75f-5x8p-qvmc, CVSS 9.3, critical, fixed 1.83.7), `@cap-js/sqlite` (==2.2.2, critical), `@cap-js/postgres` (==2.2.2, critical), `@cap-js/db-service` (==2.10.1, critical), `mbt` (==1.2.48, critical).
- Corpus update: added 2 organic eval holdout files to skillscan-corpus (SUP-049, PSV-059).
Sources:
- Aikido (Mini Shai-Hulud SAP CAP): https://www.aikido.dev/blog/mini-shai-hulud-has-appeared
- Socket (SAP CAP npm): https://socket.dev/blog/sap-cap-npm-packages-supply-chain-attack
- OX Security (SAP Shai-Hulud): https://www.ox.security/blog/shai-hulud-sap-supply-chain-attack-npm/
- StepSecurity (SAP CAP): https://www.stepsecurity.io/blog/a-mini-shai-hulud-has-appeared
- Onapsis (SAP CAP): https://onapsis.com/blog/sap-cap-mini-shai-hulud-supply-chain-attack/
- Sysdig TRT (CVE-2026-42208): https://www.sysdig.com/blog/cve-2026-42208-targeted-sql-injection-against-litellms-authentication-path-discovered-36-hours-following-vulnerability-disclosure
...
- BleepingComputer (LiteLLM SQLi): https://www.bleepingcomputer.com/news/security/hackers-are-exploiting-a-critical-litellm-pre-auth-sqli-flaw/
- LiteLLM release v1.83.7-stable: https://github.com/BerriAI/litellm/releases/tag/v1.83.7-stable
## 2026-04-26
