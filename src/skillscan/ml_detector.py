...
def _get_llm() -> Any | None:
    ...
    try:
        _llm_cache = Llama(
            ...
            n_threads=4,
            verbose=False,
            logits_all=True,  # required for logprobs → logit_confidence (#205)
        )
...
def _extract_logit_confidence(
    ...
    Adaptation note (#205, generative model): the original PR described
    softmaxing "benign vs malicious token logits at the verdict position".
    The v4 generative detector emits a JSON object whose `verdict` value is
    the literal string "benign" or "malicious". The verdict token is the
    first generated token that begins the verdict word — under the GGUF
    tokenizer this surfaces as a token starting with "ben" or "mal". We scan
    the logprobs stream left-to-right for that token, then softmax the
    chosen-token logprob against the best "benign"/"malicious" alternative in
    top_logprobs to recover a continuous P(predicted_verdict) ∈ [0, 1]. This
    is a faithful mapping of the binary-classifier intuition onto the
    generative JSON flow.

    Returns None when the logprobs payload is absent or doesn't contain a
    verdict-starting token (graceful degradation: callers leave
    logit_confidence=None and no filtering occurs).
    """
    import math
